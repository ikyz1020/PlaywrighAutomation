...
- Name: tests\api.spec.js >> Invalid Token - 401
- Location: tests\api.spec.js:88:5
...
```ts
  ...
  86  | // });
  87  | 
  88  | test('Invalid Token - 401', async ({ request }) => {
  89  | 
> 90  |     const response = await request.get(
      |                                    ^ TypeError: apiRequestContext.get: Invalid URL
  91  |         'YOUR_PROTECTED_ENDPOINT',
  92  |         {
  93  |             headers: {
  94  |                 Authorization: 'Bearer invalid_token_123'
  95  |             }
  96  |         }
  97  |     );
  98  | 
  99  |     console.log('Status:', response.status());
  100 |     console.log('Response:', await response.text());
  101 | 
