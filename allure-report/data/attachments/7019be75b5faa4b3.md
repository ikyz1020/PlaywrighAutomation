...
- Name: tests\api.spec.js >> Unauthorized Request - 401
...
```ts
  ...
  86 | // });
  87 | 
  88 | test('Unauthorized Request - 401', async ({ request }) => {
  89 | 
> 90 |     const response = await request.get(
  ...
  92 |     );
  93 | 
  94 |     console.log('Status:', response.status());
  95 |     console.log('Response:', await response.text());
  96 | 
